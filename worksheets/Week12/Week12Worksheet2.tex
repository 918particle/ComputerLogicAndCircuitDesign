...
\subsection{Send a Fourier Series through the DAC to the ADC}
...
\begin{verbatim}
#Transmission loop
sampling_frequency = 1000.0 #Units: Hertz
...
\begin{enumerate}
\item (a) What is the effect of sending the Fourier series over the line from the DAC to the ADC? (b) What kinds of distortions or oddities occur that make the ADC signal different from a square wave? \\ \vspace{2.5cm}
\item (a) What happens to your observations if you include a higher number of terms in the Fourier series? (b) While sending a relatively higher number of terms over the line, reduce the sampling frequency.  What do you notice? \vspace{2.5cm}
\end{enumerate}

\section{Bonus: Fourier Spectrum}

I wonder what this code codes?

\begin{verbatim}
N = times.shape[0]
N_f = int(N/2)
x_f = np.fft.fft(samples)
critical_frequency = sampling_frequency/2.0
df = critical_frequency/N_f
f = np.arange(0,critical_frequency,df)
x_f = x_f[0:N_f]
x_f = np.abs(x_f)
x_f_max = np.max(x_f)
x_f_dB = 20.0*np.log10(x_f/x_f_max)
plot.plot(f,x_f_dB)
plot.show()
\end{verbatim}
